...
class RelaxedBinary(RV):
    ...
    """ Binary Variable such as a sigmoid is possible? """

    def __init__(self, param, tau):
        # The output of the model that generates the location of the binary variable
        self.logits = param
    ...
    def mean(self):
        return F.sigmoid(self.logits)

    def sample(self, size=None, eps=0.001):
        if size is None:
            u = torch.zeros_like(self.logits).uniform_(eps, 1. - eps)
        elif isinstance(size, int):
            u = self.logits.new_zeros([size] + list(self.logits.shape)).uniform_(eps, 1. - eps)
        else:  # assume full shape
            u = self.logits.new_zeros(size).uniform_(eps, 1. - eps)

        # Sigmoid􏰀(log u − log(1 − u) + log α) / β
        return torch.sigmoid(torch.log(u) - torch.log(1 - u) + self.logits) / self.tau


    def log_pdf(self, x):
        logits = torch.log(x) - torch.log(1-x)
        return F.logsigmoid(logits*self.tau - self.logits)
    ...
    def quantile_concrete(self, x):
        ...
        y = F.sigmoid((torch.log(x) - torch.log(1 - x) + self.logits) / self.tau)
        return y
...
class StretchedVariable(RelaxedBinary):
    ...
    def __init__(self, dist: RelaxedBinary, support: Tensor):
        """
    ...
    def mean(self):
        return self._dist.mean()*self.scale + self.loc
    ...
    def log_cdf(self, x):
        ...
        # shrink the stretched variable
        # TODO: As per the math in Louizos division is by support[1]
        x_ = (x - self.loc) / self.max_
        # assess its cdf
    ...
    def quantile_concrete(self, x):
        """Implements the quantile, aka inverse CDF, of the 'stretched' concrete distribution"""
        return self._dist.quantile_concrete(x) * self.scale + self.loc
...
class HardBinary(RV):
    ...
    def quantile_concrete(self, x):
        """Implements the quantile, aka inverse CDF, of the 'stretched' concrete distribution"""
        return self._dist.quantile_concrete(x)

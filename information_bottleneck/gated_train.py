...
MODEL_CLASSES = {
    ...
    'distilbert': (DistilBertConfig, DistilBertGatedExplainer, DistilBertTokenizer),
}
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.tf_summary  and global_step % 20 == 0:
                    ...
                    tb_writer.add_scalar('class loss', (class_tr_loss - class_logging_loss) / 20, global_step)
                    tb_writer.add_scalar('info loss', (info_tr_loss - info_logging_loss) / 20, global_step)
...
def predict(args, model_params, model, tokenizer, eval_features, eval_dataloader):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            # Let me see what I always wanted myself to see
            ...
            # Run a tiny loop to also fill uo the predicted ratioanales output

        # TODO: Add classification measure with gold evidence (to see how well we are doing in the unsupervised setting)
    ...
    logger.info("Performance on rationales : %f F1" %
                min(classification_report(all_rationale_targets, all_rationale_results, target_names=["POS", "NEG"], output_dict=True)['NEG']['f1-score'],
                    classification_report(all_rationale_targets, all_rationale_results, target_names=["POS", "NEG"], output_dict=True)['POS']['f1-score'])
                )
...
def evaluate(args, model_params, model, tokenizer, prefix="", output_examples=False, split="val"):
    ...
    output_ratioanale_prediction_file = os.path.join(args.output_dir, "ratioanle_predictions.json")
    file_pointer = open(output_ratioanale_prediction_file, "w+")
    for e, feature in enumerate(features):
        unique_id = feature.unique_id
        tokens = feature.tokens
        token_level_rationale = all_rationale_dictionary[unique_id]
        ...
        output_dict = {"class_label" : class_label , "predicted_rationale" : rationale_text, "human_rationale" : human_rationale , "query" : query}

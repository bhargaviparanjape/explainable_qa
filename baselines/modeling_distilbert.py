...
@add_start_docstrings("""Bert Model transformer with gold attention""")
class DistilBertWithFocusedAttention(DistilBertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    """
    def __init__(self, config):
        super(DistilBertWithFocusedAttention, self).__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, labels=None, evidence_labels=None):

        # attention mask is like the input mask, except now the input exclusively contains only the gold
        attention_mask = evidence_labels
...
@add_start_docstrings("""BERT with additional noisy attention""")
class DistilBertWithAttentionPal(DistilBertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    """
    def __init__(self, config):
        super(DistilBertWithAttentionPal, self).__init__(config)
        self.num_labels = config.num_labels

        self.distilbert = DistilBertModel(config)
        self.dropout = nn.Dropout(config.seq_classif_dropout)
        self.pre_classifier = nn.Linear(2*config.dim, config.dim)
        self.classifier = nn.Linear(config.dim, config.num_labels)
    ...
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, labels=None, evidence_labels=None):
        ...
        # Pal layer can be implemented truly in the way we would like , will require larger GPU to run bert once more, maybe can be tried for albert and distillbert
        # For now maxpool over the passage wherever there are ones and attach that representation to CLS
        # attention_mask = evidence_labels
        outputs = self.distilbert(input_ids,
                            attention_mask=attention_mask,
                            head_mask=head_mask,
        ...
        focused_outputs = self.distilbert(input_ids,
                            attention_mask=evidence_labels,
                            head_mask=head_mask,
                            inputs_embeds=inputs_embeds)

        pooled_output = torch.cat((outputs[0][:, 0], focused_outputs[0][:, 0]), dim=-1)
        pooled_output = self.pre_classifier(pooled_output)  # (bs, dim)
        pooled_output = nn.ReLU()(pooled_output)  # (bs, dim)
        pooled_output = self.dropout(pooled_output)  # (bs, dim)
        logits = self.classifier(pooled_output)  # (bs, dim)
...
class DistilBertForTokenClassification(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        # The part before max query length needn't be predicted


        outputs = (logits,) + outputs[2:]  # add hidden states and attention if they are here
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            labels = labels[:, self.max_query_length:].contiguous()

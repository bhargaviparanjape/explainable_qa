...
@add_start_docstrings("""Bert Model transformer with a sequence classification/regression head on top (a linear layer on top of
                      ...
                      BERT_START_DOCSTRING,
                      BERT_INPUTS_DOCSTRING)
class BertWithFocusedAttention(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    """
    def __init__(self, config):
        super(BertWithFocusedAttention, self).__init__(config)
        self.num_labels = config.num_labels
...
class BertWithAttentionPal(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    """
    def __init__(self, config):
        super(BertWithAttentionPal, self).__init__(config)
        self.num_labels = config.num_labels

        self.bert = BertModel(config)
        #from_pretrained needs to be made better
        self.bert_pal = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        focused_outputs = self.bert(input_ids,
                            attention_mask=evidence_labels,
                            token_type_ids=token_type_ids,
                            position_ids=position_ids,
                            head_mask=head_mask,
        ...
        pooled_output = torch.cat((outputs[1], focused_outputs[1]), dim=-1)

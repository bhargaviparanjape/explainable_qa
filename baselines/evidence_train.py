...
from transformers import (WEIGHTS_NAME, BertTokenizer, RobertaTokenizer, DistilBertTokenizer, \
                          BertConfig, RobertaConfig, DistilBertConfig, \
                          AdamW, get_linear_schedule_with_warmup)

from modeling_bert import BertForTokenClassification
from modeling_distilbert import DistilBertForTokenClassification
...
def load_and_cache_examples(args, model_params, tokenizer, evaluate=False, split="train", output_examples=False):
    ...
    if evaluate:
        ...
        tensorized_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                           all_class_labels, all_cls_index, all_p_mask, all_evidence_labels,
                                           all_unique_ids)
...
def train(args, model_params, train_dataset, model, tokenizer):
    ...
    if args.evaluate_during_training:
        dataset, eval_features = load_and_cache_examples(args, model_params, tokenizer, evaluate=True, split="val",
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = predict(args, model_params, model, tokenizer, eval_features, eval_dataloader)
                        # for key, value in results.items():
                        #    tb_writer.add_scalar('eval_{}'.format(key), value, global_step)
                        if best_f1 < results:
                            logger.info("Saving model with best %s: %.2f (Acc %.2f) -> %.2f (Acc %.2f) on epoch=%d" % \
                                        (metric_name, best_f1[0] * 100, best_f1[1] * 100, results[0] * 100,
                                         results[1] * 100,
                                         epoch))
...
def predict(args, model_params, model, tokenizer, eval_features, eval_dataloader):
    all_results = []
    all_targets = []
    results = None
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            batch_hard_pred.append(hard_preds)
            all_results.extend([item for m_len, sublist in zip(mask_sums, hard_preds) for item in sublist[:m_len]])
        for i, example_index in enumerate(example_indices):
            eval_feature = eval_features[example_index.item()]
            unique_id = int(eval_feature.unique_id)
            evidence_label = list(eval_feature.evidence_label[args.max_query_length:][:mask_sums[i]])
            all_targets.extend(evidence_label)
    results = ( min(classification_report(all_targets, all_results, target_names=["POS", "NEG"],
                              output_dict=True)['NEG']['f1-score'],
        classification_report(all_targets, all_results, target_names=["POS", "NEG"],
                              output_dict=True)['POS']['f1-score']), accuracy_score(all_targets, all_results))
    return results


def evaluate(args, model_params, model, tokenizer, prefix="", output_examples=False, split="val"):
    if output_examples:
        dataset, examples, features = load_and_cache_examples(args, model_params, tokenizer, evaluate=True, split=split,
                                                              output_examples=output_examples)
    else:
        dataset, features = load_and_cache_examples(args, model_params, tokenizer, evaluate=True, split=split,
                                                    output_examples=output_examples)
    ...
    class_interner = dict((y, x) for (x, y) in enumerate(model_params['classes']))
    class_labels = [k for k, v in sorted(class_interner.items(), key=lambda x: x[1])]
    write_file = open(args.predicted_evidence_file.replace(".pkl", "_" + split + ".pkl"), "wb")
    pred_dict = {}
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        model.eval()
        batch = tuple(t.to(args.device) for t in batch)
        ...
        with torch.no_grad():
            ...
            batch_hard_pred.append(hard_preds)
            all_results.extend([item for m_len, sublist in zip(mask_sums, hard_preds) for item in sublist[:m_len]])
        batch_hard_pred = np.asarray(batch_hard_pred).squeeze(0)
        for i, example_index in enumerate(example_indices):
            eval_feature = features[example_index.item()]
            unique_id = int(eval_feature.unique_id)
            evidence_label = list(eval_feature.evidence_label[args.max_query_length:][:mask_sums[i]])
            all_targets.extend(evidence_label)
            # TODO: Since we are going to use these features downsstream, store the, keyed by eval_feature.unique_id, along with class labels and tokens:
            pred_dict[eval_feature.doc_id] = batch_hard_pred[i]
    results = (min(classification_report(all_targets, all_results, target_names=["POS", "NEG"],
                                         output_dict=True)['NEG']['f1-score'],
                   classification_report(all_targets, all_results, target_names=["POS", "NEG"],
                                         output_dict=True)['POS']['f1-score']),
    ...
    pickle.dump(pred_dict, write_file)
    logger.info("Dumping evidence predictions into file %s" % args.predicted_evidence_file)
    return results
...
def main():
    ...
    parser.add_argument("--predicted_evidence_file", type=str, default="")
    parser.add_argument("--predicted_train_evidence_file", type=str, default="")
    parser.add_argument("--predicted_eval_evidence_file", type=str, default="")
    ...
    config = config_class.from_pretrained(
        model_params["config_name"] if model_params["config_name"] else model_params["model_name_or_path"])
    config.num_labels = len(model_params['classes'])
    config.max_query_length = args.max_query_length
    tokenizer = tokenizer_class.from_pretrained(
        model_params["tokenizer_name"] if model_params["tokenizer_name"] else model_params["model_name_or_path"],
        do_lower_case=model_params["do_lower_case"])
    model = model_class.from_pretrained(model_params["model_name_or_path"],
                                        from_tf=bool('.ckpt' in model_params["model_name_or_path"]),
                                        config=config)
    ...
    if args.do_train:
        ...
        train_dataset, _ = load_and_cache_examples(args, model_params, tokenizer, evaluate=False, split="train",
                                                   output_examples=False)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # Evaluate
            result = evaluate(args, model_params, model, tokenizer, prefix=global_step, output_examples=True,

...
from evidence_utils import (read_examples, convert_examples_to_features, convert_binary_mask_rationales, \
...
def predict(args, model_params, model, tokenizer, eval_features, eval_dataloader):
    ...
    results = (classification_report(all_targets, all_results, target_names=class_labels, output_dict=True)['weighted avg']['f1-score'],
               accuracy_score(all_targets, all_results))
    evidence_report = classification_report(all_rationale_targets, all_rationale_results, target_names=["POS", "NEG"],
                              output_dict=True)

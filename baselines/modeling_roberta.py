...
@add_start_docstrings("""Roberta with focused attention models """, ROBERTA_START_DOCSTRING, ROBERTA_INPUTS_DOCSTRING)
class RobertaWithFocusedAttention(BertPreTrainedModel):
    r"""
            **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
                Labels for computing the sequence classification/regression loss.
                Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaWithFocusedAttention, self).__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                inputs_embeds=None,
                labels=None, evidence_labels=None):
...
@add_start_docstrings("""Roberta with pal attention model""", ROBERTA_START_DOCSTRING, ROBERTA_INPUTS_DOCSTRING)
class RobertaWithAttentionPal(BertPreTrainedModel):
    r"""
            **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
                Labels for computing the sequence classification/regression loss.
                Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaWithAttentionPal, self).__init__(config)
        self.num_labels = config.num_labels
